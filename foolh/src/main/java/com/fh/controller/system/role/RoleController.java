...
public class RoleController extends BaseController {
	 ...
	 * @return
	 */
	@RequestMapping(value="/toEditCondtion")
	public ModelAndView toEditCondtion(String ROLE_ID){
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		try{
			pd = this.getPageData();
			...
			mv.addObject("pd", pd);
			mv.setViewName("system/role/role_condtion_edit");
		} catch(Exception e){
			logger.error(e.toString(), e);
		}
		return mv;
	...
	public ModelAndView edit()throws Exception{
		...
		logBefore(logger, Jurisdiction.getUsername()+"修改金额提成");
		logBefore(logger, Jurisdiction.getUsername()+"修改积分提成");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		try{
			pd = this.getPageData();
			...
			FHLOG.save(Jurisdiction.getUsername(), "修改金额提成:"+pd.getString("mdeduct"));
			FHLOG.save(Jurisdiction.getUsername(), "修改积分提成:"+pd.getString("ideduct"));
	...
	public ModelAndView editD()throws Exception{
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "edit")){return null;} //校验权限
		logBefore(logger, Jurisdiction.getUsername()+"修改成为代理金额");
		logBefore(logger, Jurisdiction.getUsername()+"修改代理介绍");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		try{
			pd = this.getPageData();
			roleService.editD(pd);
			FHLOG.save(Jurisdiction.getUsername(), "修改代理条件:"+pd.getString("role_moneyduct"));
			FHLOG.save(Jurisdiction.getUsername(), "修改代理介绍:"+pd.getString("role_condition"));
			mv.addObject("msg","success");
		} catch(Exception e){
			logger.error(e.toString(), e);
			mv.addObject("msg","failed");
