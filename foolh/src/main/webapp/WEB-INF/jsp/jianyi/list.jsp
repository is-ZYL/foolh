							...
							<table id="dynamic-table" class="table table-striped table-bordered table-hover">
									...
									<tbody id="jianYiList" >
										...
										<tr v-cloak v-if="list != []"  v-for="ser in list">
											...
											<td calss="center" style="text-align: left;color:brown;width: 50px;overflow: hidden;text-overflow:ellipsis;white-space: nowrap;">{{ ser.propMsg == null ? "无":ser.propMsg}} 打不打哈接电话不就暗示的巴萨的打不打哈接电话不就暗示的巴萨的打不打哈接电话不就暗示的巴萨的打不打哈接电话不就暗示的巴萨的打不打哈接电话不就暗示的巴萨的</td>
											<td calss="center" style="text-align: center;"><span> {{ ser.propAddUserId }}</span></td>
											<td calss="center" style="text-align: center;"><span> {{ dateFormat(ser.created)}}</span></td>
											<!-- 审核状态 -->
											<td calss="center " style="text-align: center;width:60px" v-if="ser.propIsCheck == 1" @click="changeJianyiStatus(ser.shopStatus,ser.id,$event)"><a class="btn btn-mini btn-primary" title="更改审核状态">已审核</a></td>
											<td calss="center red" style="text-align: center;width:60px"  v-if="ser.propIsCheck == 0" @click="changeJianyiStatus(ser.shopStatus,ser.id,$event)"><a class="btn btn-mini btn-danger"  title="更改审核状态">未审核</a></td>
										</tr>
									</tbody>
								</table>
